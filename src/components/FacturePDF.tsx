...
export function FacturePDF({ facture, onClose }: FacturePDFProps) {
  const handlePrint = () => {
    window.print();
  };

  const montantPaye = facture.paiements?.reduce((sum: number, p: any) => sum + (p.montant || 0), 0) || 0;
  const solde = (facture.montant_ttc || 0) - montantPaye;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        {/* Document Content */}
        <div className="flex-1 overflow-y-auto p-8 bg-white" id="facture-content">
          {/* En-tête Entreprise */}
          <div className="flex justify-between items-start mb-8">
            ...
            <div className="text-right">
              <h1 className="text-3xl font-bold text-red-600 mb-2">FACTURE</h1>
              <div className="text-sm text-gray-600">
                <p><span className="font-semibold">N°:</span> {facture.numero_facture}</p>
                <p><span className="font-semibold">Date:</span> {new Date(facture.date_facture).toLocaleDateString('fr-FR')}</p>
                {facture.date_echeance && (
                  <p><span className="font-semibold">Échéance:</span> {new Date(facture.date_echeance).toLocaleDateString('fr-FR')}</p>
                )}
                {facture.commande?.numero_commande && (
                  <p><span className="font-semibold">Commande:</span> {facture.commande.numero_commande}</p>
                )}
                <p>
                  ...
                  <span className={`
                    ...
                    ${facture.statut === 'payée' ? 'text-green-600' : 
                      facture.statut === 'en_retard' ? 'text-red-600' : 'text-orange-600'}
                  `}>
                    {facture.statut}
                  </span>
                </p>
          ...
          <div className="mb-8 bg-gray-50 p-4 rounded-lg">
            ...
            <div className="text-sm text-gray-700">
              <p className="font-semibold">{facture.client?.raison_sociale || `${facture.client?.prenom} ${facture.client?.nom}`}</p>
          ...
          <div className="flex justify-end mb-8">
            <div className="w-80">
              <div className="flex justify-between py-2 border-b">
                ...
                <span className="font-semibold">{facture.montant_ht?.toFixed(2)} MAD</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-gray-600">TVA (20%):</span>
                <span className="font-semibold">{facture.montant_tva?.toFixed(2)} MAD</span>
              </div>
              <div className="flex justify-between py-3 bg-red-600 text-white px-3 rounded">
                <span className="font-bold">Total TTC:</span>
                <span className="font-bold text-xl">{facture.montant_ttc?.toFixed(2)} MAD</span>
              </div>
              ...
              {montantPaye > 0 && (
                <>
                  <div className="flex justify-between py-2 border-b mt-4">
                    ...
                    <span className="font-semibold text-green-600">-{montantPaye.toFixed(2)} MAD</span>
                  </div>
                  <div className="flex justify-between py-3 bg-gray-100 px-3 rounded mt-2">
                    <span className="font-bold">Solde restant:</span>
                    <span className={`font-bold text-xl ${solde > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {solde.toFixed(2)} MAD
                    </span>
                  </div>
                </>
          ...
          {facture.mode_paiement && (
            <div className="mb-8 bg-blue-50 border-l-4 border-blue-400 p-4">
              <p className="font-semibold text-gray-900 mb-1">Mode de paiement:</p>
              <p className="text-sm text-gray-700">{facture.mode_paiement}</p>
            </div>
          )}
          ...
          <div className="space-y-4 text-sm text-gray-600">
            ...
            <div>
              ...
              <p>{facture.conditions_paiement || 'Paiement à 30 jours'}</p>
            </div>
            {facture.date_echeance && (
              <div className="bg-yellow-50 border border-yellow-200 rounded p-3">
                <p className="font-semibold text-gray-900">⚠️ Date limite de paiement:</p>
          ...
          <div className="mt-8 bg-gray-50 p-4 rounded-lg">
            <p className="font-semibold text-gray-900 mb-2">Coordonnées bancaires</p>
            <div className="text-sm text-gray-700 space-y-1">
              <p>Banque: [Nom de votre banque]</p>
              <p>RIB: XXXX XXXX XXXX XXXX XXXX XXXX</p>
              <p>IBAN: MA XX XXXX XXXX XXXX XXXX XXXX XXXX</p>
            </div>
          </div>
          ...
          <div className="mt-12 pt-6 border-t text-center text-xs text-gray-500">
            ...
            <p className="mt-2 font-semibold">Merci de votre confiance</p>
          </div>
        </div>
      </div>
